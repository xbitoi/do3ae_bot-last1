...
function getPersistentPath(filename: string): string {
  try {
    ...
    if (fs.existsSync(parentDir)) {
      ...
      // Dynamic migration: if a file already exists in ancestral folder (/artifacts/filename),
...
const ANALYTICS_FILE = getPersistentPath("analytics.json");
const CHANNEL_STATS_FILE = getPersistentPath("channel-stats.json");

...
class TelegramBotManager:
    ...
    async def _start_bot(self):
        ...
        telegram_api_url = (os.getenv("TELEGRAM_API_URL") or os.getenv("BOT_API_URL") or "https://do3ae-telegram-proxy.khalidh2.workers.dev").strip()
        if not telegram_api_url.startswith("http://") and not telegram_api_url.startswith("https://"):
            telegram_api_url = "https://" + telegram_api_url
        base_url = telegram_api_url.rstrip("/") + "/bot"
        self.log(f"🌐 Using Telegram proxy: {base_url}", "info")
        self._app = Application.builder().token(self.bot_token).base_url(base_url).build()

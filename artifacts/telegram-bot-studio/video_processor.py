...
def _fade_alpha(t: float, duration: float, fi: float, fo_start: float) -> float:
    # Simply return 1.0 to disable fade-out, ensuring words stay visible and do not evaporate
    if fi > 0 and t < fi:
        return t / fi
    return 1.0
...
def _apply_word_effect(static_arr, t: float, duration: float, effect: str):
    ...
    # Map newly added UI options to existing supported effects
    if effect == 'typewriter':
        effect = 'reveal_rtl'
    elif effect == 'wave_cascade':
        effect = 'bounce_spring'
...
def _render_word_pil(
    ...
    if active_index >= 0 and active_index < len(all_words):
        ...
        try:
            ...
            if has_raqm:
                # RTL layout: calculate prefix width based on words prior to active index
                # Sum individual word lengths with spaces to find original LTR coordinate and mirror it
                before_w = sum(draw.textlength(reshape_arabic(w) + " ", font=font) for w in words_before) if words_before else 0
                rect_x = start_x + full_w - before_w - active_w - 6
            else:
                # LTR layout of reversed text: words are reversed, so words after active index are on the left
                words_after = words_list[active_index + 1:]
                after_w = sum(draw.textlength(reshape_arabic(w) + " ", font=font) for w in words_after) if words_after else 0
                rect_x = start_x + after_w - 6
...
def _merge_and_process_sync(
    ...
    font_name = settings.get("font", "Tajawal")
...
def get_font_path(font_name: str) -> str:
    ...
    font_file = font_map.get(font_name, "tajawal.ttf")

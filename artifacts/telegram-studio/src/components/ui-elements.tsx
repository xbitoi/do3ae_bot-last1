  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="flex items-center gap-3">
        <div className={cn(
          "relative overflow-hidden rounded-lg w-11 h-9 border border-border shrink-0 cursor-pointer shadow-sm hover:border-primary/50 transition-colors bg-gradient-to-r from-red-500 via-green-500 to-blue-500"
        )}>
          <input 
            type="color" 
            value={isRandom ? "#3B82F6" : value}
            onChange={(e) => onChange(e.target.value)}
            className="absolute -inset-3 w-20 h-16 cursor-pointer opacity-0"
          />
          {!isRandom && (
            <div className="absolute inset-0 pointer-events-none" style={{ backgroundColor: value }} />
          )}
          {isRandom && (
            <div className="absolute inset-0 pointer-events-none flex items-center justify-center bg-black/40 text-white font-bold text-[10px]">
              🎲
            </div>
          )}
        </div>
        ...
        <button
          type="button"
          onClick={() => onChange(isRandom ? "#3B82F6" : "random")}
          className={cn(
            "px-2.5 py-2 rounded-xl border text-xs font-bold transition-all shrink-0",
            isRandom

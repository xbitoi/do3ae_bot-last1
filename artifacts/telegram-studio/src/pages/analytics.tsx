...
import {
  Video, Users, Bot, RefreshCw, TrendingUp, Eye, ThumbsUp,
  MessageCircle, Share2, BarChart2, AlertCircle,
...
function UsersAnalytics({ data }: { data: FBData }) {
...
function BotAnalyticsTab({ data }: { data: BotData }) {
  ...
  const platforms: Record<string, { icon: React.ElementType; color: string; barColor: string }> = {
    "يوتيوب": { icon: Video, color: "text-red-400", barColor: "bg-red-500" },
    "فيسبوك": { icon: Users, color: "text-blue-400", barColor: "bg-blue-500" },
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="rounded-[2rem] bg-card border border-border shadow-2xl overflow-hidden">
        ...
        <div className="p-5 grid grid-cols-2 gap-3">
          {[
            { key: "youtube",  label: "يوتيوب",  icon: Video,  activeClass: "bg-red-500/10 border-red-500/20",  iconClass: "text-red-400" },
            { key: "facebook", label: "فيسبوك",  icon: Users, activeClass: "bg-blue-500/10 border-blue-500/20", iconClass: "text-blue-400" },
...
const PLATFORMS: { id: Platform; label: string; icon: React.ElementType; accent: string }[] = [
  { id: "youtube",  label: "يوتيوب",  icon: Video,  accent: "red"  },
  { id: "facebook", label: "فيسبوك",  icon: Users, accent: "blue" },
...
export function Analytics() {
  ...
  const renderContent = () => {
    ...
    if (active === "facebook" && "page" in current) return <UsersAnalytics data={current as FBData} />;

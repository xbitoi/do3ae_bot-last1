...
function ConnectionStatusCard({ settings, onSettingChange }: { settings: any; onSettingChange: (ns: any) => void }) {
  ...
  useEffect(() => {
    if (settings?.offlineMode) {
      setStatus({ type: "none", checked: true, checking: false, error: "⚠️ تم تعطيل الاتصال بالإنترنت يدويًا وضع الطيران نشط" });
      return;
    }
    fetchStatus();
  ...
  return (
    <div className="relative group rounded-[2rem] bg-card border border-border shadow-2xl overflow-hidden p-5 sm:p-6">
      ...
      <div className="relative z-10 space-y-4">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            <div className={cn("p-2 rounded-xl border shadow-inner transition-colors", settings?.offlineMode ? "bg-amber-500/10 border-amber-500/30" : bgColor)}>
              {settings?.offlineMode ? <WifiOff className="w-4 h-4 text-amber-500" /> : <Icon className={cn("w-4 h-4", iconColor, isChecking && "animate-spin")} />}
            </div>
            <div>
              <p className="text-xs font-black text-foreground/70 uppercase tracking-widest">حالة الاتصال بالإنترنت</p>
              <p className={cn("text-sm font-bold mt-0.5", settings?.offlineMode ? "text-amber-500 animate-pulse" : (isChecking ? "text-primary" : isNone ? "text-destructive" : "text-success"))}>
                {settings?.offlineMode
                  ? "⚠️ وضع الطيران نشط (غير متصل)"
                  : isChecking
                    ? "جاري فحص الاتصال…"
                    : isNone
          ...
          <button
            onClick={recheck}
            disabled={rechecking || isChecking || settings?.offlineMode}
        ...
        {/* زر إيقاف التطبيق واتصاله بالإنترنت */}
        {settings && (
          <div className="pt-3 border-t border-border/40 flex items-center justify-between">
            <div className="space-y-0.5">
              <span className="text-xs font-bold text-foreground">🔌 إيقاف اتصال التطبيق بالإنترنت (وضع عدم الاتصال)</span>
              <p className="text-[10px] text-muted-foreground/50">يقطع كل الاتصالات الخارجية ويوقف البوت تلقائياً ومباشرة</p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
                checked={settings.offlineMode ?? false}
                onChange={(e) => onSettingChange({ ...settings, offlineMode: e.target.checked })}
        ...
        <div className="relative z-10 px-6 sm:px-8 pb-6 sm:pb-8 space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              { key: "facebookToken", label: "فيسبوك", color: "orange" },
            ].map(({ key, label, color }) => (
  ...
  const handleFetchModels = async () => {
    ...
    try {
      ...
      if (data.models?.length) {
        ...
        toast({ title: "تنبيه", description: "أدخل مفتاح Gemini أولاً في قسم المفاتيح المتقدمة", variant: "destructive" });
      }
    } catch {
      toast({ title: "خطأ", description: "تعذّر جلب النماذج، تحقق من المفتاح في قسم المفاتيح المتقدمة", variant: "destructive" });
    } finally {
        ...
        {tab === 'text' && (
           <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
             <Select label="نوع الخط" value={settings.font} onChange={(v: string) => setSettings({...settings, font: v})} options={[
               {label: "🎲 خط عشوائي (يتغير تلقائياً مع كل فيديو)", value: "random"},
               {label: "خط الرقعة - Reqaa (رصين ومتناسق)", value: "Reqaa"},
               {label: "خط النسخ - Naskh (الأكثر وضوحاً وقراءة)", value: "Naskh"},
               {label: "خط الديواني - Diwani (جمالي وانسيابي)", value: "Diwani"},
               {label: "خط الديواني الجلي - DiwaniJali (مزخرف وفخم)", value: "DiwaniJali"},
               {label: "خط الثلث - Thuluth (أم الخطوط وزينتها)", value: "Thuluth"},
               {label: "خط النستعليق - Nastaliq (الفارسي الأنيق الممدود)", value: "Nastaliq"},
               {label: "خط الشكسته - Shikasteh (المعقد والرشيق)", value: "Shikasteh"}
             ]} />
             ...
             <div className="pt-3 border-t border-warning/20 space-y-4">
               ...
               <Select label="🔤 تأثير ظهور الكلمات" value={settings.wordEffect ?? "random"} onChange={(v: string) => setSettings({...settings, wordEffect: v})} options={[
                 ...
                 {label: "تلاشي الكلمات غير النشطة 🌫️", value: "fade_others"},
                 {label: "تلاشي الكلمات السابقة فقط 👵", value: "fade_past"},
                 {label: "الكلمة النشطة فقط 🎯", value: "only_active"},
                 {label: "تدفق كاريوكي تدريجي 🎤", value: "karaoke_flow"},
                 {label: "تلاشي ناعم للكلمة ✨", value: "fade_smooth"},
                 {label: "وميض نيون خاطف ⚡", value: "neon_flicker"},
                 {label: "نبض القلب 💓", value: "heartbeat"},
        ...
        {tab === 'audio' && (
           <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
             ...
             <div className="space-y-2">
               ...
               {voicesOpen && (
               <div className="space-y-4 pl-2 border-r border-emerald-500/20 pr-3 animate-in fade-in slide-in-from-top-2 duration-200">
                 {/* Voice Provider Selector */}
                 <div className="flex gap-2">
                   <div className="flex-1 py-1.5 rounded-lg text-xs font-bold border border-blue-500 text-blue-400 bg-blue-500/20 text-center flex items-center justify-center gap-2">
                     <span className="opacity-80">🌐</span> Edge TTS 
                     <span className="text-[10px] bg-blue-500/30 px-1.5 py-0.5 rounded text-white">+ gTTS</span>
                   </div>
                 </div>
                 ...
                 <button
                   ...
                   className={cn(
                     ...
                     (!settings.ttsVoice || settings.ttsVoice === "random")
                       ? "border-primary bg-primary/15 text-foreground shadow-[0_0_12px_rgba(99,102,241,0.2)]"
                       : "border-border/50 bg-black/20 text-muted-foreground hover:border-border hover:text-foreground"
                   )}
                 >
                   ...
                   <span className="flex flex-col items-start">
                     <span className="font-black text-sm">عشوائي (من كل الأصوات)</span>
                 ...
                 <p className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-widest pt-1">أصوات ذكور — Edge TTS</p>
                 <div className="grid grid-cols-1 gap-1.5">
                   {[
                      { value: "ar-SA-HamedNeural",  label: "الشيخ حامد (Hamed)", desc: "سعودي — تلاوة خاشعة كأئمة الحرم، دافئة وروحانية جداً" },
                      { value: "ar-YE-SalehNeural",  label: "الشيخ صالح (Saleh)", desc: "يمني — صوت خاشع حزين بطيء مليء بالروحانية والأثر القلبي" },
                      { value: "ar-IQ-BasselNeural",  label: "الشيخ باسل (Bassel)", desc: "عراقي — صوت جهوري فخم، غليظ وعميق جداً ذو هيبة بالغة" },
                      { value: "ar-KW-FahedNeural",  label: "المقرئ فهد (Fahed)", desc: "كويتي — نبرة هادئة متزنة بطيئة تناسب الأدعية والأوراد" },
                      { value: "ar-EG-ShakirNeural",  label: "القارئ شاكر (Shakir)", desc: "مصري — صوت شاب واضح وفصيح وقوي النبرة" },
                      { value: "ar-AE-HamdanNeural",  label: "القارئ حمدان (Hamdan)", desc: "إماراتي — نبرة هادئة خفيفة وناعمة للأذكار والخشوع" },
                    ].map((v) => (
                     <button
                       key={v.value}
                       ...
                       className={cn(
                         ...
                         settings.ttsVoice === v.value
                           ? "border-blue-500/60 bg-blue-500/10 text-foreground shadow-[0_0_12px_rgba(59,130,246,0.15)]"
                           : "border-border/50 bg-black/20 text-muted-foreground hover:border-border hover:text-foreground"
                       )}
                     >
                       <span className={cn("text-lg", settings.ttsVoice === v.value ? "opacity-100" : "opacity-40")}>🎙️</span>
                       <span className="flex flex-col items-start">
                         <span className="font-black text-sm">{v.label}</span>
                         <span className="text-[11px] opacity-70">{v.desc}</span>
                       </span>
                       {settings.ttsVoice === v.value && (
                         <span className="mr-auto text-blue-400 text-xs font-black">✓</span>
                       )}
                     </button>
                   ))}
                 </div>

                 <p className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-widest pt-1">أصوات إناث — Edge TTS</p>
                 <div className="grid grid-cols-1 gap-1.5">
                   {[
                     { value: "ar-SA-ZariyahNeural",  label: "القارئة زارية (Zariyah)", desc: "سعودية — نبرة وقورة واضحة وممتازة لقراءة الأدعية بخشوع وسكينة" },
                     { value: "ar-YE-MaryamNeural",   label: "القارئة مريم (Maryam)", desc: "يمنية — صوت شجي فيه مسحة حزن ووقار مليء بالروحانية الفائقة" },
                     { value: "ar-EG-SalmaNeural",    label: "القارئة سلمى (Salma)", desc: "مصرية — صوت ناعم هادئ ورقيق يناسب الأذكار والأوراد الروحانية" },
                   ].map((v) => (
                     <button
                       key={v.value}
                       onClick={() => setSettings({...settings, ttsVoice: v.value})}
...
const FONT_FAMILY_MAP: Record<string, string> = {
  random: "'Naskh', 'Amiri', serif",
  Reqaa: "'Reqaa', 'Aref Ruqaa', serif",
  Naskh: "'Naskh', 'Amiri', serif",
  Diwani: "'Diwani', serif",
  DiwaniJali: "'DiwaniJali', serif",
  Thuluth: "'Thuluth', 'Katibeh', serif",
  Nastaliq: "'Nastaliq', 'Noto Nastaliq Urdu', serif",
  Shikasteh: "'Shikasteh', serif",
};
          ...
          <span className="text-foreground/80 font-semibold">
            {(() => {
              const labels: Record<string, string> = {
                random: "عشوائي 🎲",
                none: "بدون تأثير ⛔",
                fade_others: "تلاشي الكلمات غير النشطة 🌫️",
                fade_past: "تلاشي الكلمات السابقة فقط 👵",
                only_active: "الكلمة النشطة فقط 🎯",
                karaoke_flow: "تدفق كاريوكي تدريجي 🎤",
                fade_smooth: "تلاشي ناعم للكلمة ✨",
                neon_flicker: "وميض نيون خاطف ⚡",
                heartbeat: "نبض القلب 💓",
                zoom_pop: "تكبير بوب 💥",
                bounce_spring: "ارتداد نابضي 🏀",
                slide_up: "صعود من الأسفل ⬆️",
                slide_down: "نزول من الأعلى ⬇️",
                swing_right: "دخول انسيابي 🌊",
                glow_pulse: "وميض متنفس 💫",
                reveal_rtl: "كشف من اليمين 🪟",
                typewriter: "كاتب آلي ⌨️",
                wave_cascade: "موجة متتالية 〰️",
                matrix_rain: "مصفوفة رقمية 🔢",
                shimmer: "بريق متحرك ✦",
                spin_in: "دوران ثلاثي 🌀",
                scramble: "تشابك حروف 🔀"
              };
              return labels[settings.wordEffect ?? "random"] || (settings.wordEffect ?? "عشوائي");
            })()}
          </span>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 animate-in fade-in slide-in-from-bottom-8 duration-700 pb-20">
      <div className="lg:col-span-5 space-y-8 lg:space-y-10">
        <ConnectionStatusCard settings={settings} onSettingChange={handleSettingChange} />
        <BotStatusMiniCard
          status={status}
